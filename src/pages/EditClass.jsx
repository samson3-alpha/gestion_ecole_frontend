...
import { addClass, getClasses, updateClass } from '../bd/classes';
...
function EditClass() {
  ...
  const [classes, setClasses] = useState(getClasses().data)
  const { id } = useParams();
  const classe = classes.find((c) => c.id === parseInt(id));
  const [nom, setNom] = useState(classe.nom);
  const [type, setType] = useState(classe.type);
  const [serie, setSerie] = useState(classe.serie);
  const [categorie, setCategorie] = useState(classe.categorie);
  ...
  const handleSubmit = (e) => {
    ...
    if (nom&&categorie&&type&&serie){
      ...
      updateClass(parseInt(id), updatedData);
  ...
  return (
    ...
    <div className="max-w-md mx-auto bg-white p-6 rounded-lg shadow-lg">
      <h2 className="text-2xl font-semibold text-center mb-6">Modifier la classe</h2>
